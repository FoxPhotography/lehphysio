...
export const PlayGame: React.FC<PlayGameProps> = ({
  ...
  const handleCopyInviteUrl = () => {
    ...
    showToast('تم نسخ رابط الدعوة! شاركه مع زملائك للانضمام فوراً 🔗');
      ...
      <div className="games-panel animate-fade-in">
        <div className="glass-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <div>
              <span className="badge-tag">بانتظار اللاعبين ⏳</span>
              <h2 style={{ fontSize: '20px', fontWeight: 900, marginTop: '4px' }}>غرفة تحدي التشريح أونلاين</h2>
            </div>
          ...
          <div style={{ textAlign: 'center', margin: '2rem 0' }}>
            ...
            <p style={{ fontSize: '11px', color: 'var(--text-secondary)', marginTop: '0.5rem' }}>اضغط على الكود لنسخه ومشاركته</p>
          </div>

          <div className="small-invite-row">
            <input type="text" readOnly value={inviteUrl} className="small-invite-input" />
            <button className="btn-primary mini-compact" onClick={handleCopyInviteUrl}>نسخ الرابط</button>
          </div>

          <div style={{ marginTop: '2rem' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 800, marginBottom: '0.75rem' }}>اللاعبون المتصلون ({room.players.length})</h4>
          ...
          <div style={{ marginTop: '2.5rem' }}>
            {isHost ? (
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)' }}>
                ...
                <span>بانتظار أن يبدأ الهوست اللعبة...</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="games-panel animate-fade-in">
        <div className="glass-card">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--card-border)', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
            <div>
              <span className="badge-tag">الجولة {room.currentRound} من {room.rounds}</span>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: 'var(--orange)', marginTop: '4px' }}>تخمين الهيكل التشريعي 🤔</h3>
            </div>
            <div style={{ textAlign: 'center', background: 'rgba(255, 106, 0, 0.1)', border: '1px solid var(--orange)', borderRadius: '12px', padding: '6px 14px' }}>
              <span style={{ fontSize: '18px', fontWeight: 900, color: 'var(--amber)', fontFamily: 'sans-serif' }}>{room.roundWinner ? ' استراحة' : timeLeft}</span>
            </div>
            <div style={{ textAlign: 'left' }}>
              <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>نقاطي</span>
          ...
          <div className="glass-card" style={{ background: '#0A0A0A', marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '0.5rem' }}>
              <span className="badge-tag">النوع: {question.type === 'muscle' ? 'عضلة 💪' : 'عظمة 🦴'}</span>
              <span className="badge-tag" style={{ background: 'rgba(255,255,255,0.06)', color: '#fff' }}>تلميح {currentHintIdx + 1} من 4</span>
            </div>
            <p style={{ fontSize: '16px', fontWeight: 700, lineHeight: 1.6, color: '#fff' }}>
              {room.roundWinner ? `تم الكشف عن الإجابة: ${question.structure}` : currentHint}
            </p>
          </div>
          ...
          {!room.roundWinner && (
            <form onSubmit={handleSubmitGameAnswer} style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
              <input
                type="text"
                className="pl-input"
                placeholder="اكتب اسم الهيكل التشريعي بالعربية أو اللاتينية..."
          ...
          {room.roundWinner && (
            <div className="glass-card" style={{ background: 'rgba(255, 106, 0, 0.05)', borderColor: 'var(--orange)', textAlign: 'center', marginBottom: '1.5rem' }}>
              <h4 style={{ color: 'var(--amber)', fontWeight: 800, fontSize: '15px' }}>
                {room.roundWinner === 'timeout' ? ' انتهى وقت الجولة!' : ` أجاب ${room.roundWinner} إجابة صحيحة!`}
              </h4>
              <p style={{ marginTop: '0.25rem', fontSize: '13px' }}>الهيكل التشريعي الصحيح: <strong>{question.structure}</strong></p>
              {isHost && (
                <button className="btn-primary mini" onClick={handleNextRound} style={{ marginTop: '1rem' }}>
                  {room.currentRound < room.rounds ? 'أكمل للجولة التالية ➡️' : 'عرض النتيجة النهائية 🏆'}
                </button>
              )}
            </div>
          )}
          ...
          <div className="glass-card" style={{ background: '#0A0A0A', maxHeight: '180px', overflowY: 'auto' }}>
            <h4 style={{ fontSize: '12px', fontWeight: 800, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>سجل الإجابات 💬</h4>
    ...
    return (
      <div className="games-panel animate-fade-in">
        <div className="glass-card" style={{ textAlign: 'center', padding: '2.5rem' }}>
          ...
          <h2 style={{ fontSize: '24px', fontWeight: 900, color: 'var(--orange)', marginTop: '1rem' }}>انتهى التحدي الجماعي!</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: '0.5rem 0 2.5rem' }}>توج {room.finalWinner} بطلاً للغرفة.</p>
          ...
          <div className="leaderboard-list-container" style={{ maxWidth: '400px', margin: '0 auto 2rem' }}>
            {sorted.map((p: any, idx: number) => (
              <div key={p.username} className="leaderboard-row-item" style={{ background: '#0A0A0A' }}>
                ...
                <span className="row-xp-badge">{p.score} نقطة</span>
              </div>
            ))}
          </div>

          <button className="btn-primary" onClick={handleLeaveGameRoom}>العودة لقائمة التحديات</button>
        </div>
      </div>
    );
  }
